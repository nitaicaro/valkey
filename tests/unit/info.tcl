...
start_server {tags {"info"} overrides {save "" forkless-options-supported yes}} {
    test {INFO threadsave metrics show default values when no save is running} {
        ...
        # When no threadsave is running, time metrics should be -1, counts should be 0
        assert_match "*threadsave_current_item_seconds:-1*" $info
        assert_match "*threadsave_current_queue_length:0*" $info
        assert_match "*threadsave_queue_length_target:0*" $info
        assert_match "*threadsave_estimated_seconds_remaining:-1*" $info
        assert_match "*threadsave_dbentries_queued:0*" $info
        assert_match "*threadsave_dbentries_processed:0*" $info
    }
    ...
    test {INFO threadsave metrics are present during active save} {
        ...
        assert_match "*threadsave_current_queue_length:*" $info
        assert_match "*threadsave_queue_length_target:*" $info
        assert_match "*threadsave_estimated_seconds_remaining:*" $info
        assert_match "*threadsave_dbentries_queued:*" $info
        assert_match "*threadsave_dbentries_processed:*" $info
        
        # Verify queue_length_target has a reasonable value
        assert {[s threadsave_queue_length_target] > 0}
    ...
    test {INFO threadsave cumulative metrics increase during save} {
        ...
        set queued1 [s threadsave_dbentries_queued]
        set processed1 [s threadsave_dbentries_processed]
        
        # Wait more
        after 200
        
        set queued2 [s threadsave_dbentries_queued]
        set processed2 [s threadsave_dbentries_processed]
    ...
    test {INFO threadsave current_item_seconds is counted} {
        ...
        # Wait 1 second - should be in middle of processing first item
        after 1000
    ...
    test {INFO threadsave estimated_seconds_remaining is reasonable} {
        ...
        set processed [s threadsave_dbentries_processed]
    ...
    test {INFO threadsave metrics show default values after save completes} {
        ...
        # After save completes, time metrics should be -1, counts should be 0
        assert_match "*threadsave_current_item_seconds:-1*" $info
        assert_match "*threadsave_current_queue_length:0*" $info
        assert_match "*threadsave_queue_length_target:0*" $info
        assert_match "*threadsave_estimated_seconds_remaining:-1*" $info
